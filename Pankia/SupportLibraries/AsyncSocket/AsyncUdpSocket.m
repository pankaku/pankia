...
@public
	...
	long tag;
	/////////////////////////////////
	// Defined TTL variable. By Nakatsuka.
	/////////////////////////////////
	int ttl; // User definition.
}
...
// Added bind check method. by Nakatsuka.
- (BOOL)isBinded
{
	return theFlags & kDidBind;
}
...
////////////////////////////////////////////////
// Added this method for SPI block. By Nakatsuka
////////////////////////////////////////////////
- (BOOL)sendData:(NSData *)data toHost:(NSString *)host port:(UInt16)port withTimeout:(NSTimeInterval)timeout tag:(long)tag ttl:(int)ttl
{
	if((data == nil) || ([data length] == 0)) return NO;
	if(theFlags & kForbidSendReceive) return NO;
	if(theFlags & kDidClose) return NO;
...
- (void)doSend:(CFSocketRef)theSocket
{
	if(theCurrentSend != nil)
	{
		...
		if([self canAcceptBytes:theSocket])
		{
			...
			////////////////////////////////////////////////
			// Change TTL limit. By Nakatsuka
			////////////////////////////////////////////////
			int ttl		= theCurrentSend->ttl;
			int old_ttl = 0;
			int ret		= 0;
			socklen_t len = sizeof(old_ttl);
			ret = getsockopt(theNativeSocket, IPPROTO_IP, IP_TTL, (char *)&old_ttl, &len);
			ret = setsockopt(theNativeSocket, IPPROTO_IP, IP_TTL, (char *)&ttl, len);
			////////////////////////////////////////////////
			...
			// Change TTL limit. By Nakatsuka
			////////////////////////////////////////////////
			ret = setsockopt(theNativeSocket, IPPROTO_IP, IP_TTL, (char *)&old_ttl, len);
			////////////////////////////////////////////////
